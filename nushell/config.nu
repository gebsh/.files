...
# Wrap this in `do` to prevent leaking internal imports and local variables.
do --env {
	use std/util 'path add'
	use ./modules/internal [ theme ]

	const HOME_DIR = ('~' | path expand)

	# Add XDG environment variables because there are tools that unconditionally use them instead of
	# OS-specific directories leading to inconsistencies and polluting the home directory.
	#
	...
	$env.LS_COLORS = (
		[
			[type, fg];
			['*', $theme.palette.text]
			[di, $theme.palette.yellow]
			[ln, $theme.palette.blue]
			[so, $theme.palette.mauve]
			[pi, $theme.palette.pink]
			[ex, $theme.palette.text]
			[bd, $theme.palette.peach]
